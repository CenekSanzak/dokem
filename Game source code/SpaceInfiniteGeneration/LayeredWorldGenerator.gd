...
export var generate_on_cloud := true

func _ready() -> void:
	start_time = OS.get_ticks_msec()
	if not generate_on_cloud:
		generate()
...
func convertStringToVector2(pair: String) -> Vector2:
	var vector2key = pair
	vector2key.erase(vector2key.find("("),1)
	vector2key.erase(vector2key.find(")"),1)
	vector2key.erase(vector2key.find(","),1)
	var x = vector2key.left(vector2key.find(" "))
	var y = vector2key.right(vector2key.find(" "))
	return Vector2(x,y)



func _on_HTTPRequest_request_completed( result, response_code, headers, body):
	var start_time2 = OS.get_ticks_msec()
	_sectors = parse_json(body.get_string_from_utf8())
	for key in _sectors.keys():
		var vector2key = convertStringToVector2(key)
		_sectors[vector2key] = _sectors[key]
		_sectors.erase(key)
	for value in _sectors.values():
		if "seeds" in value:
			for i in range(value.seeds.size()):
				value.seeds[i] = Vector2(value.seeds[i][0], value.seeds[i][1])
		if "planet" in value:
			value.planet.position = Vector2(value.planet.position[0], value.planet.position[1]) 
		if "moons" in value:
			for i in range(value.moons.size()):
				value.moons[i].position = Vector2(value.moons[i].position[0], value.moons[i].position[1]) 
		if "travel_lanes" in value:
			for i in range(value.travel_lanes.size()):
				value.travel_lanes[i].source = Vector2(value.travel_lanes[i].source[0], value.travel_lanes[i].source[1]) 
				value.travel_lanes[i].destination = Vector2(value.travel_lanes[i].destination[0], value.travel_lanes[i].destination[1]) 
		if "asteroids" in value:
			for i in range(value.travel_lanes.size()):
				value.asteroids[i].position = Vector2(value.asteroids[i].position[0], value.asteroids[i].position[1]) 
