...
func _on_HTTPRequest_request_completed( result, response_code, headers, body):
	...
	for value in _sectors.values():
		...
		if "moons" in value:
			for moon in value.moons:
				value.moons.moon.position = Vector2(value.moons.moon.position[0], value.moons.moon.position[1])
		if "travel_lanes" in value:
			for lane in value.travel_lanes:
				value.travel_lanes.lane.position = Vector2(value.travel_lanes.lane.position[0], value.travel_lanes.lane.position[1])
		if "asteroids" in value:
			for asteroid in value.asteroids:
				value.asteroids.asteroid.position = Vector2(value.asteroids.asteroid.position[0], value.asteroids.asteroid.position[1])
